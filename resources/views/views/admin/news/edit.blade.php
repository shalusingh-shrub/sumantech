    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card border-0 shadow-sm p-4 mb-3">
                <div class="mb-3">
                    ...
                    <input type="text" name="title" id="titleInput" class="form-control" placeholder="Enter title" value="{{ old('title', $news->title) }}" required>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Slug</label>
                    <input type="text" name="slug" id="slugInput" class="form-control" value="{{ old('slug', $news->slug) }}">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="content" id="contentEditor">{{ old('content', $news->content) }}</textarea>
                </div>
                <div class="mb-3">
        ...
        <div class="col-md-4">
            <div class="sidebar-card">
                ...
                <div class="d-flex gap-3 mb-3">
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="status" id="statusActive" value="active" {{ $news->is_published ? 'checked' : '' }}>
                        <label class="form-check-label" for="statusActive">Active</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="status" id="statusInactive" value="inactive" {{ !$news->is_published ? 'checked' : '' }}>
                        <label class="form-check-label" for="statusInactive">Inactive</label>
                    </div>
                </div>
                <label class="form-label fw-semibold mb-1">Publish Date</label>
                <input type="date" name="publish_date" class="form-control mb-3" value="{{ old('publish_date', $news->publish_date?->format('Y-m-d') ?? date('Y-m-d')) }}">
                <button type="submit" class="publish-btn">Update</button>
            </div>
            <div class="sidebar-card">
                <h6>News Type</h6>
                <div class="form-check mb-2">
                    <input class="form-check-input" type="radio" name="news_type" id="typeNews" value="news" {{ $news->category === 'news' ? 'checked' : '' }}>
                    <label class="form-check-label" for="typeNews">News</label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="news_type" id="typeEvent" value="event" {{ $news->category === 'event' ? 'checked' : '' }}>
                    <label class="form-check-label" for="typeEvent">Event</label>
                </div>
                <div id="eventDateWrap" class="mt-3" style="{{ $news->category === 'event' ? '' : 'display:none;' }}">
            ...
            <div class="sidebar-card">
                <h6>Pin to Home</h6>
                <div class="form-check mb-2">
                    <input class="form-check-input" type="radio" name="pin_to_home" id="pinYes" value="yes" {{ $news->pin_to_home ? 'checked' : '' }}>
                    <label class="form-check-label" for="pinYes">Yes</label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="pin_to_home" id="pinNo" value="no" {{ !$news->pin_to_home ? 'checked' : '' }}>
            ...
            <div class="sidebar-card">
                ...
                @if($news->image)
                    <img src="{{ $news->image_url }}" class="img-fluid rounded mb-2" style="max-height:120px;" onerror="this.onerror=null;this.style.opacity='0.3'">
                @endif
