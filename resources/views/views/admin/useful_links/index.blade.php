{{-- File: resources/views/admin/useful_links/index.blade.php --}}
...
<div class="card data-card"><div class="card-body p-0">
    <table class="table table-hover mb-0 align-middle">
        <thead style="background:#f8f9fa;"><tr><th>#</th><th>Title</th><th>URL</th><th>Category</th><th>Status</th><th>Created By</th><th>Updated By</th><th>Actions</th></tr></thead>
        <tbody>
            @forelse($links as $i => $item)
            <tr>
                <td>{{ $links->firstItem() + $i }}</td>
                <td><strong>{{ $item->title }}</strong></td>
                <td><a href="{{ $item->url }}" target="_blank" class="text-truncate d-inline-block" style="max-width:180px;">{{ $item->url }}</a></td>
                <td>{{ $item->category ?? '-' }}</td>
                ...
                <td>
                    <a href="{{ route('admin.useful-links.edit', $item) }}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                    <form action="{{ route('admin.useful-links.destroy', $item) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete?')">@csrf @method('DELETE')
                        <button class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></button>
                    </form>
                </td>
            </tr>
            @empty
            <tr><td colspan="8" class="text-center py-4 text-muted">Koi link nahi mila.</td></tr>
            @endforelse
        </tbody>
    </table>
