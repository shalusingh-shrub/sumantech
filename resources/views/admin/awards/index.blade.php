...
<div class="card data-card mb-3">
    <div class="card-body py-3">
        <form method="GET" class="row g-2 align-items-end">
            <div class="col-md-9"><input type="text" name="search" class="form-control" placeholder="Search by title..." value="{{ request('search') }}"></div>
        ...
        <table class="table table-hover mb-0 align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><strong>{{ Str::limit($award->title, 40) }}</strong></td>
                    ...
                    <td>
                        <a href="{{ route('admin.awards.participants', $award) }}" class="badge bg-info text-white text-decoration-none">
                            {{ $award->participants()->count() }} Participants
                        </a>
                    </td>
                    <td><span class="badge {{ $award->is_active ? 'bg-success' : 'bg-secondary' }}">{{ $award->is_active ? 'Active' : 'Inactive' }}</span></td>
                    <td>
                        @if($award->createdBy ?? false)<small class="text-muted"><i class="fas fa-user me-1"></i>{{ $award->createdBy->name }}</small><br>@endif
                    ...
                    <td>
                        <div class="d-flex flex-column gap-1">
                            <div class="d-flex gap-1">
                                ...
                                <form action="{{ route('admin.awards.destroy', $award) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete?')">
                                    @csrf @method('DELETE')
                                    <button class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></button>
                                </form>
                            </div>
                            <a href="{{ route('admin.awards.participants', $award) }}" class="btn btn-sm btn-info text-white py-1" style="font-size:11px;">
