...
@section('title', 'Edit Marks Template')
@section('content')
<div class="content-area">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0 fw-bold" style="color:#1a2a6c;">
      <i class="fas fa-edit me-2"></i>Edit Marks Template
    </h4>
  ...
  @php
    $subjects = is_string($template->subjects) ? json_decode($template->subjects, true) : ($template->subjects ?? []);
    $grades   = is_string($template->grade_standards) ? json_decode($template->grade_standards, true) : ($template->grade_standards ?? []);
    $subjects = $subjects ?: [];
    $grades   = $grades ?: [];
  @endphp

  <form method="POST" action="{{ route('admin.marks.templates.update', $template) }}">
    @csrf @method('PUT')
    <div class="row g-4">
            ...
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <select name="course_id" class="form-select" onchange="setCourseNameFromSelect(this)">
                  ...
                  <option value="{{ $c->id }}" data-name="{{ $c->name }}"
                    {{ $template->course_id == $c->id ? 'selected' : '' }}>
                    {{ $c->name }}
                  </option>
                  @endforeach
                </select>
              ...
              <div class="col-md-6">
                ...
                <input type="text" name="course_name" id="courseName" class="form-control"
                       value="{{ old('course_name', $template->course_name) }}" required>
              </div>
            </div>
          </div>
        </div>
              ...
              <table class="table align-middle mb-0" id="subjectsTable">
                ...
                <tbody id="subjectRows">
                  ...
                  <tr>
                    ...
                    <td><input type="text" name="subjects[{{ $i }}][name]" class="form-control"
                               value="{{ $subject['name'] ?? '' }}" required></td>
                    <td><input type="number" name="subjects[{{ $i }}][max]" class="form-control"
                               value="{{ $subject['max_marks'] ?? 100 }}" min="1" onchange="calcTotal()" required></td>
                    <td>
                  ...
                  <tr>
                    ...
                    <td><input type="number" name="subjects[0][max]" class="form-control" value="100" min="1" onchange="calcTotal()" required></td>
                    <td>
                      <button type="button" class="btn btn-sm btn-outline-danger" onclick="removeRow(this)">
                        <i class="fas fa-trash"></i>
                      </button>
                ...
                <tfoot style="background:#f8f9fa;">
                  <tr>
                    ...
                    <td><span id="totalMax" class="fw-bold" style="color:#1a2a6c;">0</span></td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
            ...
            <div class="table-responsive">
              <table class="table align-middle mb-0">
                ...
                <tbody id="gradeRows">
                  ...
                  <tr>
                    <td class="px-3"><input type="text" name="grades[{{ $i }}][grade]" class="form-control"
                               value="{{ $grade['grade'] ?? '' }}" style="width:80px;"></td>
                    <td><input type="number" name="grades[{{ $i }}][min]" class="form-control"
                               value="{{ $grade['min'] ?? 0 }}" min="0" max="100" style="width:80px;"></td>
                    <td><input type="number" name="grades[{{ $i }}][max]" class="form-control"
                               value="{{ $grade['max'] ?? 100 }}" min="0" max="100" style="width:80px;"></td>
                    <td>
                      <select name="grades[{{ $i }}][result]" class="form-select" style="width:140px;">
                        @foreach(['Pass','Fail','1st Division','2nd Division','3rd Division','Distinction'] as $r)
                        <option value="{{ $r }}" {{ ($grade['result'] ?? '') == $r ? 'selected' : '' }}>{{ $r }}</option>
                        @endforeach
                      </select>
                    </td>
                    <td>
                  ...
                  <tr>
                    ...
                    <td>
                      <select name="grades[0][result]" class="form-select" style="width:140px;">
      ...
      <div class="col-12">
        <div class="card border-0 shadow-sm p-4" style="border-radius:12px;">
          ...
          <textarea name="notes" class="form-control" rows="3"
                    placeholder="e.g. Minimum 40% required to pass each subject">{{ old('notes', $template->notes) }}</textarea>
        </div>
      </div>
      ...
      <div class="col-12">
        ...
        <a href="{{ route('admin.marks.templates.index') }}" class="btn btn-secondary px-4 ms-2">Cancel</a>
      </div>
...
<script>
let subjectCount = {{ count($subjects) ?: 1 }};
let gradeCount   = {{ count($grades) ?: 1 }};
