...
<div class="card border-0 shadow-sm p-4" style="border-radius:10px;">
    <form action="{{ route('admin.good-luck-messages.update', $goodLuckMessage) }}" method="POST">@csrf @method('PUT')
        <div class="mb-3">
            ...
            <input type="text" name="title" class="form-control" value="{{ old('title', $goodLuckMessage->title) }}" required>
        </div>
        <div class="mb-3">
            <label class="form-label fw-semibold">Message *</label>
            <textarea name="message" class="form-control" rows="6" required>{{ old('message', $goodLuckMessage->message) }}</textarea>
        </div>
        <div class="row">
